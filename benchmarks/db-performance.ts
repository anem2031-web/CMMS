 ...
 * - ALL measurements go through Drizzle ORM (via server/db.ts functions)
 * - Each query is executed 5 times to collect statistical measurements
 * - Raw SQL cases (#7 and #8) executed via Drizzle sql helper
 */
...
// ─── Benchmark runner ─────────────────────────────────────────────────────────
async function benchmarkFn(name: string, fn: () => Promise<unknown>, runs = 5) {
  console.log(`  ⏱️  Running (${runs} runs): ${name}`);
...
// ─── EXPLAIN plan helper via Drizzle sql helper ───────────────────────────────
async function getExplainPlan(rawSql: string): Promise<unknown[]> {
  const drizzle = await getDb();
  if (!drizzle) return [];
  try {
    const result = await drizzle.execute(sql.raw(`EXPLAIN ${rawSql}`));
    // Drizzle mysql2 execute returns [rows, fields] array
    return (Array.isArray(result) ? result[0] : result) as unknown[];
  } catch {
    return [];
  }
}
...
async function runAnalysis() {
  console.log('\n🔍 Starting DB Performance Benchmark via Drizzle ORM (server/db.ts)...\n');

  // Resolve a real PO id for getPOItems
  const drizzle = await getDb();
  let firstPoId = 1;
  if (drizzle) {
    try {
      const result = await drizzle.execute(sql.raw('SELECT id FROM purchase_orders LIMIT 1'));
      const rows = Array.isArray(result) ? result[0] : result;
      const first = (rows as any[])[0];
      if (first?.id) firstPoId = Number(first.id);
    } catch { /* use default */ }
  }
  console.log(`  → Using PO id=${firstPoId} for getPOItems()\n`);

  // ── 1. Benchmark REAL db.ts functions (Drizzle ORM path) ─────────────────
  const benchmarks = [
    await benchmarkFn('db.getTickets()',                   () => getTickets()),
    await benchmarkFn('db.getTickets({ status: "open" })', () => getTickets({ status: 'open' })),
    await benchmarkFn('db.getDashboardStats()',             () => getDashboardStats()),
    await benchmarkFn('db.getTechnicianPerformance()',      () => getTechnicianPerformance()),
    await benchmarkFn('db.getPurchaseOrders()',             () => getPurchaseOrders()),
    await benchmarkFn('db.getPOItems()',                    () => getPOItems(firstPoId)),
  ];

  // ── 2. Raw SQL via Drizzle sql helper (NOT mysql2/promise) ────────────────
  benchmarks.push(
    await benchmarkFn(
      'SELECT * FROM tickets WHERE assignedToId = ? LIMIT 50',
      async () => {
        const d = await getDb();
        if (!d) return;
  ...
  console.log('\n📋 Capturing EXPLAIN plans via Drizzle sql helper...\n');

  const explainTargets: { name: string; sql: string }[] = [
    {
      name: 'db.getTickets()',
  ...
  const explainMap: Record<string, unknown[]> = {};
  for (const t of explainTargets) {
    console.log(`  📋 EXPLAIN: ${t.name}`);
  ...
  console.log('📌 PROOF — Functions called from server/db.ts:');
  benchmarks.forEach(b => console.log(`   ✓ ${b.name} → avg ${b.avg_ms}ms`));
  process.exit(0);
}
